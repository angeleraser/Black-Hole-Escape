...
class Rocket {
  ...
  keyupEvent(key) {
    let { letter: keyLetter, isPressingKey } = key;
  ...
  evaluateKeysPressed(key1, key2) {
    let { isPressingKey: pressinKeyOne } = key1,
      { isPressingKey: pressinKeyTwo } = key2;
  ...
  increaseDistance() {
    if (this.distance < safeZoneLength * 0.5) {
      this.distance += 12;
    }
  }
...
const timing = (lapse1, lapse2 = lapse1 + 1) =>
  currentPercentage >= lapse1 && currentPercentage <= lapse2;
...
const gameOver = () => {
  ...
  currentPercentage = 0;
  ROCKET.distance = Number((safeZoneLength * 0.4).toFixed(0));
    ...
    failSound.pause();
    {
      ...
      gameInterval = setInterval(() => {
        ...
        currentPercentage = Number(
          ((currentSongTime * 100) / theBigBlack_mp3.duration).toFixed(1)
        );
        ...
        if (timing(0, 2))
          levelPassScreenText(
            "Keep the distance from Black Hole by tapping alternately the red buttons S & D"
          );
        // LEVEL 1 from 5% to 28%
        if (timing(4))
          levelPassScreenText(`${span("- STAGE 1 -")}Tap S & D 200 times.`);
        ...
        if (timing(97) && keysToPress == 0)
          levelPassScreenText("¡You escaped from the Black Hole!", 2000);
        // END...
